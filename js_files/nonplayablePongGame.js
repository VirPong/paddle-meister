...
function getGameState(player1, player2){
  ...
  if(player1 > yPos){
    if((player1 + paddleHeight) < HEIGHT){
      yPos = player1;
    }else{
      yPos = HEIGHT - paddleHeight;
    }
  }else if(player1 < yPos){
    if((player1 > 0 )){
      yPos = player1;
    }else{
      yPos = 0;
  ...
  //player two statement
  if(player2 > arrowYPos){
    if((player2 + paddleHeight) < HEIGHT){
      arrowYPos = player2;
    }else{
      arrowYPos = HEIGHT - paddleHeight;
    }
  }else if(player2 < arrowYPos){
    if((player2 > 0 )){
      arrowYPos = player2;
    }else{
      arrowYPos = 0;
  ...
  dataList = new Array(yPos, arrowYPos, x, y);
  return dataList;
}
...
function drawPlayers(){
  ...
  paddle1(arrowYPos);
   window.onkeydown = function(e){
	...
	if(keyCode == qwertyW && yPos> 20){//offset top??? or 20... WE DONT KNOW!!
		yPos -= 10;
	}
